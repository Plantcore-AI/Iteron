...
This page is generated from the argument parser, so every shipped flag and subcommand appears here. `core --help` is the same contract for the exact build you have installed, and `iteron --version` identifies it by commit and build date.
