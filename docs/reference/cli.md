...
| `--allow-code` | Enable code execution (bash/build/test). ON by default; a trusted `~/.core/config.json` "allow_code": false, a project `.core/config.json` "allow_code": false, or `--mode plan` tightens it back off. The command runs with your own user authority unless `--confine`. |
