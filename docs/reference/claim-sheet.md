# Iteron v0.0.20 evidence-bound claim sheet
...
| --- | --- | --- |
| The workspace version is `0.0.20`. | `Cargo.toml` workspace package version; matching Iteron package entries in `Cargo.lock`. | Source version, not proof that every release target is published. |
| The optimization census schema is version `4`. | `governance/optimization-census.json` field `schema_version`. | Census format only. |
| The census contains `2,971` independent candidate rows: `2,084` runtime-settable, runtime-applied, and externally addressed; `0` unaddressed; `0` binding-required; `887` invariant/read-only. | `governance/optimization-census.json` summary fields. | Addressability census, not performance or quality. |
