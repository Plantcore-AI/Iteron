# Iteron v0.0.22 evidence-bound claim sheet
...
| --- | --- | --- |
| The workspace version is `0.0.22`. | `Cargo.toml` workspace package version; matching Iteron package entries in `Cargo.lock`. | Source version, not proof that every release target is published. |
