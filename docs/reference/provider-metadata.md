...
Some provider facts cannot be discovered from an account-scoped API. Core ships a
...
`<human-label>+sha256:<canonical-content-digest>`; Core recomputes all snapshot and
...
must be owned by the effective user with owner-only mode bits. On Windows Core
...
LocalSystem, or Administrators. Core reads only through that validated,
...
duplicate it. Core labels a snapshot stale after 30 complete days; a capture
