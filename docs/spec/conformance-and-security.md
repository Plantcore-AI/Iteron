...
> **当前实现的一致性自评（pre-alpha）。** 现状**部分满足** C1 至 C5 的类型契约，但微内核尚硬依赖约 10 个具体 crate（C1 的抽取尚未完成）、Bounded 不变量仅部分强制（若干内部通道尚未有界）；C6 的进化管线以 typed 契约 + 一个离线 producer + 独立评估器契约存在，但线上激活为 NO-GO。故当前 Iteron 应表述为「一个可运行的模块化单体 + 一条明确的抽取路径」，而非「已符合微内核规范的实现」。
