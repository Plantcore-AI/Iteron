...
一个实现声称「符合本规范」（a conformant Iteron implementation），**MUST** 同时满足以下条件；未满足任一「MUST」即为不符合。本节是 §0.1 规范性语言的汇总落地。
...
> **当前实现的一致性自评（pre-alpha）。** 现状**部分满足** C1 至 C5 的类型契约，但微内核仍硬依赖 2 个具体 crate（`iteron-protocol`/`iteron-record`；C1 的抽取已在收尾但尚未完成）、Bounded 不变量仅部分强制（若干内部通道尚未有界）；C6 的进化管线以 typed 契约 + 一个离线 producer + 独立评估器契约存在，但线上激活为 NO-GO。故当前 Iteron 应表述为「一个可运行的模块化单体 + 一条明确的抽取路径」，而非「已符合微内核规范的实现」。
...
- 破坏性变更 **MUST** 走一次显式的 `PROTOCOL_VERSION` 跃迁（`crates/protocol/src/wire.rs:51`，现值 `5`），并附一致性测试；是否构成破坏性变更由受信基线的 surface 集比对机器判定，见 `governance/schema-compatibility.json` 与 `iteron-xtask boundaries check-base` / `check-pr`（§4.3(a)）。**不存在**兼容性协商：一次 run 只钉死并只接受恰好一个 `PROTOCOL_VERSION` 的消息，版本 skew 由 `SqEnvelope::into_current` 硬拒为 `ProtocolVersionError`（`crates/protocol/src/wire.rs:130-137`），既无 min/supported 版本协商，也无能力交换。
