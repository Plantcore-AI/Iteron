...
mod tests {
    ...
    #[test]
    fn a_duplicate_request_id_is_refused_rather_than_stranding_the_first_caller() {
        // Replacing the entry for an id already in flight loses the first caller's deadline and
        // method: its reply can never be matched, and it waits forever with nothing to sweep it.
        // Ids are the caller's to allocate, so a collision is a bug worth surfacing, not absorbing.
        let mut p = PendingRequests::default();
        p.issue(7, "definition", 0, 1_000).unwrap();

        assert_eq!(
            p.issue(7, "hover", 500, 1_000),
            Err(LspError::DuplicateRequestId { id: 7 })
        );
        assert_eq!(p.in_flight(), 1);

        // The original entry must survive intact -- same method, same original deadline.
        let fired = p.expire(1_000);
        assert_eq!(
            fired,
            vec![Expired {
                id: 7,
