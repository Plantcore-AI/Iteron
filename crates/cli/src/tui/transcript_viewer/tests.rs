...
fn user(id: u64, text: &str) -> block::Block {
    block::Block::new(id, block::BlockKind::User(text.into()))
}
...
fn index_reconciles_revisions_and_eviction_and_searches_unicode() {
    ...
    viewer.open("你好", &blocks);
    assert_eq!(viewer.results, vec![1]);
    assert_eq!(viewer.selected_id, Some(1));

    blocks[0].kind = block::BlockKind::User("changed 東京".into());
    blocks[0].touch();
    blocks.remove(1);
...
fn navigation_raw_copy_and_filters_are_deterministic_and_bounded() {
    ...
    let mut viewer = Viewer::default();
    viewer.open("needle", &blocks);
    assert_eq!(viewer.export_ids(ExportScope::Filtered), Some(vec![2]));
...
fn search_matches_beyond_the_old_prefix_and_surfaces_any_unindexed_block() {
    ...
    let mut viewer = Viewer::default();
    viewer.open("late-needle", &blocks);
    assert_eq!(viewer.results, vec![1]);
...
#[test]
fn live_blocks_reindex_without_a_revision_bump() {
    let mut blocks = vec![block::Block::new(
        9,
    ...
    let mut viewer = Viewer::default();
    viewer.open("after", &blocks);
    assert!(viewer.results.is_empty());
    let block::BlockKind::Tool(card) = &mut blocks[0].kind else {
        unreachable!()
    };
...
fn row_cache_handles_combining_wide_tiny_and_zero_surfaces() {
    ...
    for width in [1, 2, 4] {
        ...
        assert!(
            rows.iter()
                .all(|line| crate::render::line_width(line) <= width)
        );
    }
...
fn result_count_and_query_bytes_are_hard_bounded() {
    let blocks = (0..(MAX_RESULTS + 20))
        ...
        .collect::<Vec<_>>();
    let mut viewer = Viewer::default();
    viewer.open(&"x".repeat(MAX_QUERY_BYTES * 2), &blocks);
