...
/// distinction decides.
fn bare_image_path_spans(text: &str) -> Vec<(usize, usize, PathBuf)> {
...
fn handle_composer_paste(app: &mut App, workspace: &Path, pasted: &str) {
...
fn dropped_image_reference(raw: &str) -> Result<Option<PathBuf>, image_input::ImageInputError> {
...
fn decode_dropped_escapes(token: &str) -> Option<String> {
    if cfg!(windows) {
        return (!token
            .chars()
...
/// that does not exist.
fn attach_bare_image_paths(app: &mut App, workspace: &Path) {
...
fn note_image_refusal(app: &mut App, path: &Path, error: &image_input::ImageInputError) {
