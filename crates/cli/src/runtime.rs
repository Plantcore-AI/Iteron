    ...
    async fn drive_admitted(
        ...
        loop {
            ...
            for (idx, intent) in overflow_pure {
                ...
                // Dispatched inline, where the journal IS writable, so this read crosses the
                // boundary in the order the boundary intends: the intent is fsynced before the
                // executor is entered (I-42).
                let ticket =
                    self.open_tool_call_effect(turn_id, idx, &tu_ui, Capability::ReadOnly)?;
                ...
                self.commit_admitted_tool_result(ticket, &tu_ui.name, &r, 0)?;
                any_error |= r.is_error;
