...
//! rendering, so adding a flag without regenerating fails the build (`cargo test -p core-xtask`
...
pub fn check(root: &Path) -> Result<()> {
    ...
    if actual != expected {
        bail!(
            "generated file {} drifted from the argument parser in {CLI_SOURCE}; run `cargo run --locked -p core-xtask -- docs generate`",
            path.display()
        );
    }
    Ok(())
...
fn render_sources(source: &str, modules: &[(&str, &str)]) -> Result<String> {
    ...
    writeln!(
        out,
        "<!-- Generated from `{CLI_SOURCE}` by `cargo run --locked -p core-xtask -- docs generate`. Do not edit this file directly. -->\n"
    )
    .unwrap();
...
`core --version` identifies it by commit and build date.\n"
    )
    .unwrap();
