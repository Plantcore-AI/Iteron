...
impl Subcommand {
    fn rows(&self, out: &mut Vec<(String, String)>) {
        let mut invocation = format!("iteron {}", self.path.join(" "));
...
fn render_sources(source: &str, modules: &[(&str, &str)]) -> Result<String> {
    ...
    .unwrap();
    writeln!(out, "The executable name is `iteron`.\n").unwrap();
    writeln!(out, "```text").unwrap();
    writeln!(out, "iteron [OPTIONS] [TASK] [COMMAND]").unwrap();
    writeln!(out, "```\n").unwrap();
    writeln!(
        out,
        "This page is generated from the argument parser, so every shipped flag and subcommand \
appears here. `iteron --help` is the same contract for the exact build you have installed, and \
`iteron --version` identifies it by commit and build date.\n"
    ...
    write_table(
        ...
        [
            ...
            (
                "`-V`, `--version`".to_string(),
                "Print the bare `iteron <version>`.".to_string(),
            ),
            (
                "`--version`".to_string(),
