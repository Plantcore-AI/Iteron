//! Offline projection for the non-authoritative `core-tunables` registry and pure resolver contract.
//!
...
fn render_markdown() -> Result<String> {
    ...
    writeln!(
        out,
        "- Registry schema: `core-tunables/v{REGISTRY_SCHEMA_VERSION}` (revision `{REGISTRY_REVISION}`)"
    )?;
    writeln!(
        out,
        "- Pure resolution input schema: `core-tunables-resolution/v{RESOLUTION_SCHEMA_VERSION}`"
    )?;
    writeln!(
        out,
        "- Canonical digest: `{}:{}`",
